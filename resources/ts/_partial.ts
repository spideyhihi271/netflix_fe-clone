...
class Render {
    public header(): void {
        ...
        `
        let headerContentMobile: string = `
    ...
    protected gender(genders): string {
        let content = '';
    ...
    protected bannerSlider(pictures): string {
        let content = '';
        pictures.map(pic => {
            content += `
        ...
        new Swiper(".mySwiper", {
            ...
            on: {
                autoplayTimeLeft(s, time, progress) {
                    (progressCircle as HTMLElement).style.setProperty("--progress", (1 - progress) as any);
    ...
    protected movieItem(data): string {
        let content = '';
        data.map( movie => {
            content +=
    ...
    public movieList({ data, title, filter }): string {
        let content = '';
        content =
            `
    ...
    public moviesUseForList(data): void {
        let content = '';
        data.map(movie => {
            content +=
                `
    ...
    protected optionItem(values): string {
        let content = '';
        values.map(value => {
    ...
    protected selectItem(select): string {
        let content =
            `
    ...
    private deviceItem(data): string {
        let devices = [
            {
                name: 'Mobile',
    ...
    private packItem(data, isActive?: boolean): string {
        let content =
            `
    ...
    public packList(data): void {
        let content = '';
        data.map((pack, idx) => {
            if (idx === 0) content += this.packItem(pack, true);
            else content += this.packItem(pack);

        })
        packListContainer.innerHTML = content;
    }
    public packInPayment(data): void {
        let content =
            `
    ...
    protected detailCastInMovie(data): string {
        let content = '';
        data.map(cast => {
            content += 
                `
    ...
    public detailWatchBox(data): void {
        console.log(data);
        let content = `
    ...
    public detailInfoMovie(data): void {
        let content = 
        `
